...
use bf_primitives::consts::WORD_SIZE;

use crate::air::BfAirBuilder;
...
impl<T> Word<T> {
    ...
    pub fn extend_var<AB: BfAirBuilder<Var = T>>(var: T) -> Word<AB::Expr> {
        Word([AB::Expr::ZERO + var, AB::Expr::ZERO, AB::Expr::ZERO, AB::Expr::ZERO])
    }

    // /// Extends a variable to a word.
    // pub fn extend_expr<AB: BfAirBuilder<Expr = T>>(expr: T) -> Word<AB::Expr> {
...
impl<T: FieldAlgebra> Word<T> {
    ...
    #[must_use]
    pub fn zero<AB: BfAirBuilder<Expr = T>>() -> Word<T> {
